...
- Horizons: **swing (days to weeks) and position (weeks to months)** for the core, plus **intraday strategies with a few trades a day** (e.g. opening-range breakout), which are in scope. HFT and market-making are out of scope: that's the territory of firms like Jane Street and HRT.
  ...
  - The per-order minimum still caps trade count: at IBKR Fixed, each round trip costs at least $2. An intraday edge must be measured per trade in dollars, after commissions **and** slippage.
...
## Commands
```bash
uv sync                                                        # add --group research for vectorbt/lightgbm/jupyter
uv run pytest                                                  # ~115 tests, <3s, no network
...
uv run python -m horizon_trader.backtest.run [--plan tiered]   # every sleeve + combined + SPY, 2008 to today
...
- **Execution** (`execution/`):
  ...
  - `PaperBroker` is the in-memory simulator, with the cost models in `costs.py`: IBKR Fixed ($0.005/share, $1 minimum, 1% of trade value maximum) is the default; Tiered ($0.0035/share up to 300k shares/month, $0.35 minimum, exchange/clearing fees not modeled) is the alternative. Neither model yet includes the fees charged on sells (SEC $20.60 per $1M sold, FINRA TAF $0.000195/share), which matter for short, frequent trades. Verified 2026-09-24.
...
## What the evidence says so far (2008 to 2026, IBKR Fixed pricing)
| Sleeve | CAGR | Sharpe | Max DD | Verdict |
|---|---|---|---|---|
| position_momentum (ETF 12-1 rotation) | 9.8% | 0.76 | -18% | Core. Only the 12-month lookback is robust; 6-month/top-2 blew up to a -45% drawdown |
| position_volmanaged (SPY, Moreira-Muir) | 9.6% | 0.80 | -21% | Most robust (Sharpe 0.66-0.80 across the grid, 1.00 out-of-sample from 2013). 0.81 correlated with SPY |
| position_trend (TSMOM, inverse-vol) | 4.8% | 0.70 | -15% | Stable but **0.85 correlated with momentum**, so largely redundant |
| swing_meanrev (RSI(2)) | 2.2% | 0.41 | -20% | Weak at every setting and costly. Lowest correlation to the others |
...
- **ORB:** best realistic setup is top-1, 1 ATR stop, stop-limit entry at the trigger, and a larger account or IBKR Lite. That's roughly SPY-like Sharpe with no market correlation. The next test is the IBKR paper account, not more backtests.
